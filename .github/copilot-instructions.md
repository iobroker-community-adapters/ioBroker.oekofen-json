...
This adapter connects OekoFEN Pellematic pellet heating systems (and related devices such as solar modules, layer storages, and sterling engines) to ioBroker via the OekoFEN JSON API (Pelletronic Touch interface). The adapter reads all available datapoints from the controller and creates ioBroker objects dynamically at startup. It polls the OekoFEN controller at a configurable interval.

## Adapter-Specific Context

- **Adapter Name**: oekofen-json
- **Primary Function**: Poll OekoFEN Pellematic heating system JSON API and expose all datapoints as ioBroker states
- **Key Dependencies**: `axios` (HTTP polling), `@iobroker/adapter-core` (adapter base)
- **Authentication**: Password embedded directly in URL path (`http://<ip>:<port>/<password>/all`)
- **Connection Type**: Local network (LAN) polling — no cloud dependency
- **Data Source**: HTTP GET polling at a user-configured interval (default 30 seconds)
- **Configuration Requirements**:
  - `oekofenIp`: IP address of the OekoFEN controller
  - `oekofenPort`: TCP port (default 8080)
  - `oekofenPassword`: Password used as path segment in the URL (encrypted in ioBroker)
  - `myRequestInterval`: Polling interval in seconds

### OekoFEN API Patterns

- All data is fetched via `GET http://<ip>:<port>/<password>/all?` (initial scan, full metadata) and `GET http://<ip>:<port>/<password>/all` (subsequent polls, values only)
- The API returns a JSON object with top-level keys representing subsystems (e.g., `pe1`, `hk1`, `ww1`). The `forecast` key is skipped.
- Each subsystem contains datapoints with properties: `val`, `format`, `factor`, `min`, `max`, `unit`
- The `factor` field is used to scale integer values (e.g., temperature stored as `215` with factor `0.1` = `21.5°C`)
- The `?C` unit string returned by the API should be converted to `°C`
- Datapoints whose keys start with `L_` are **read-only**
- Datapoints whose keys end with `_info` should be **skipped**
- Write operations use `GET http://<ip>:<port>/<password>/<subsystem>.<key>=<value>` where value is already divided by factor
- The controller may return HTTP 401 if polled too rapidly (rate limit ~2.5 seconds between requests)
- After 10 consecutive failed updates, the adapter disables itself

### State Management

- `info.connection`: Boolean state indicating connectivity to the OekoFEN controller
- `info.rescan`: Button state to re-run the initial scan and recreate missing datapoints
- `info.update`: Button state to trigger an immediate data update
- All states are created with `setObjectNotExistsAsync` and updated with `setStateAsync`

### Error Handling Specifics

- On initial scan failure: log error, set `info.connection` to false, disable adapter
- On repeated update failures (> 10): disable adapter with an error message
- HTTP 401 from OekoFEN indicates rate limiting, not an authentication failure
...
[CUSTOMIZE: OekoFEN Pellematic-specific patterns]

- **Factor Scaling**: All numeric values from the OekoFEN API may include a `factor` field. When reading: `displayValue = rawValue * factor`. When writing: `rawValue = Math.round(displayValue / factor)`. Always store the factor in the ioBroker object's `native.factor` field.
- **Read-only Detection**: Keys starting with `L_` are read-only. Do not subscribe to or write these states.
- **Datapoint Skipping**: Keys ending with `_info` carry auxiliary metadata and must be skipped when creating states.
- **Top-level Key Skipping**: The `forecast` top-level key should always be skipped (weather data not manipulable).
- **URL Construction**: Build the request URL as `http://<oekofenIp>:<oekofenPort>/<oekofenPassword>`. Write operations append `/<subsystem>.<key>=<value>` to this base URL.
- **Rate Limiting**: The OekoFEN controller enforces a ~2.5-second minimum between requests. Avoid polling more frequently. HTTP 401 responses typically indicate rate limiting, not an authentication failure.
- **Failure Counter**: Track consecutive update failures. After 10 failures in a row, disable the adapter and log an error prompting the user to check the heater.
- **Initial 10-second Delay**: Always wait 10 seconds after adapter startup before the first connection attempt to allow ioBroker to stabilise.
- **latin1 Encoding**: All axios (or fetch) requests to the OekoFEN API must use `latin1` response encoding — the controller does not use UTF-8.
- **Unit Conversion**: The API returns `?C` for degrees Celsius. Always convert this to `°C` when storing in ioBroker.
