...
class OekofenJson extends utils.Adapter {
    ...
    async initialScan(url) {
        ...
        try {
            ...
            if (response.status === 200 && typeof response.data === 'object') {
                this.log.debug(
                    '[initialScan_axios.get] got HTTP/200 response, call parseDataOnStartupAndCreateObjects with response.data',
                );
                ...
                this.log.debug(
                    `[initialScan] set updateDataInterval to ${Number.parseInt(this.config.myRequestInterval) * 1000}`,
                );
                updateDataInterval = setInterval(
                    async () => await this.updateData(url),
                    Number.parseInt(this.config.myRequestInterval) * 1000,
                );
            } else {
    ...
    async updateData(url) {
        ...
        try {
            ...
            if (response.status === 200 && typeof response.data === 'object') {
                this.log.debug(
                    '[updateData_axios.get] got HTTP/200 response, call parseDataAndSetValues with response.data',
                );
            ...
            //Set connection to false in case of errors
            this.log.debug(
                '[updateData_axios.get.catch] error while request has occured, setting info.connection to false',
            );
            this.setStateAsync('info.connection', { val: false, ack: true });
            ...
            if (failedConnectCounter > 10) {
                this.log.error(
                    '[updateData_axios.get.catch] failed to get data 10 in a row. Disabling adapter. Please check your heater.',
                );
                this.disable();
            }
        }
    ...
    async parseDataOnStartupAndCreateObjects(jsonData) {
        ...
        if (jsonDataLength > 50) {
            this.log.error(
                `[parseDataOnStartupAndCreateObjects] jsonDataLength is too big (${
                    jsonDataLength
                }) - Adapter exiting now.`,
            );
            //Set connection to false in case of errors
            this.log.debug(
                '[parseDataOnStartupAndCreateObjects] error while parsing data has occured, disable adapter.',
            );
            this.setStateAsync('info.connection', { val: false, ack: true });
            this.disable();
            return;
        ...
        for (const key of Object.keys(jsonData)) {
            ...
            for (const innerKey of Object.keys(jsonData[key])) {
                ...
                if (
                    innerKey !== 'name' &&
                    (typeof jsonData[key][innerKey].val === 'number' || !isNaN(Number(jsonData[key][innerKey].val)))
                ) {
                ...
                if (!innerKey.endsWith('_info')) {
                    ...
                    if (!innerKey.startsWith('L_')) {
                        this.subscribeStates(`${key}.${innerKey}`);
                    }
                }
            }
        }
    ...
    async parseDataAndSetValues(jsonData, instanceObject) {
        ...
        if (jsonDataLength > 50) {
            this.log.error(
                `[parseDataAndSetValues] jsonDataLength is too big (${jsonDataLength}) - Adapter exiting now.`,
            );
            //Set connection to false in case of errors
                ...
                try {
                    ...
                    this.getObject(`${key}.${innerKey}`, function (err, obj) {
                        ...
                        if (obj && obj.native.factor) {
                            instanceObject.setStateAsync(`${key}.${innerKey}`, {
                                val: tNewVal * obj.native.factor,
                                ack: true,
                            });
                        } else {
            ...
            if (dataPoint.native.factor) {
                ...
                if (dataPoint.max) {
                    ...
                    if (realValue > realMax) {
                        this.log.error(
                            `Value ${state.val} for dataPoint ${id}is bigger than allowed max (${dataPoint.max})`,
                        );
                ...
                if (dataPoint.min) {
                    ...
                    if (realValue < realMin) {
                        this.log.error(
                            `Value ${state.val} for dataPoint ${id}is smaller than allowed min (${dataPoint.min})`,
                        );
                ...
                if (await this.sendUpdateToOekofen(id, realValue)) {
                    this.log.debug(
                        `state ${id} changed: value ${state.val} (realValue=${realValue}) (ack = ${state.ack})`,
                    );
